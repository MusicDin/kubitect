...
func TestHostUri_Empty(t *testing.T) {
	uri, err := hostUri(modelconfig2.Host{})
	assert.NoError(t, err)
	assert.Equal(t, "qemu:///system", uri)
}

func TestHostUri_Local(t *testing.T) {
	h := modelconfig2.MockLocalHost(t, "local", false)
...
func TestHostUri_Remote(t *testing.T) {
	h := modelconfig2.MockRemoteHost(t, "remote", false, false)
...
func TestHostUri_Remote_Verify(t *testing.T) {
	h := modelconfig2.MockRemoteHost(t, "remote", false, true)
	pkey := h.Connection.SSH.Keyfile
...
func TestIsDefault(t *testing.T) {
	lh := modelconfig2.MockLocalHost(t, "local", false)
	rh := modelconfig2.MockRemoteHost(t, "remote", true, false)

	hosts := []modelconfig2.Host{lh, rh}
...
func TestDefaultHost_NoDefaultHostSet(t *testing.T) {
	lh := modelconfig2.MockLocalHost(t, "local", false)
	rh := modelconfig2.MockRemoteHost(t, "remote", false, false)
...
func TestDefaultHost_EmptyList(t *testing.T) {
	_, err := defaultHost([]modelconfig2.Host{})
