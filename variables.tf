variable "config_type" {
  type        = string
  description = "Configuration type (yaml, tf) used by the project."
  default     = "yaml"

  validation {
    condition     = contains(["yaml", "tf"], var.config_type)
    error_message = "Wrong config type. Possible values are: [\"yaml\", or \"tf\"]."
  }
}
...
variable "config_path" {
  ...
  validation {
    ...
    error_message = "Config file does not exist on provided location."
  }
}
